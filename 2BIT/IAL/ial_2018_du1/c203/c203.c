...
*/
	...
	if (q == NULL)													//invalid input handling
		queueError(QERR_INIT);
	else
	{
		for (int i = 0; i < QUEUE_SIZE; ++i)	//initializing array with *
		{
			q->arr[i] = '*';
		}
		q->f_index=0;
...
*/
	...
	if (queueEmpty(q) != 0)						//execute error function if the queue is empty
		queueError(QERR_FRONT);
	else
	{
		*c = q->arr[q->f_index];				//value of the address c is pointing to, is set to value from the front of queue
	}
}
...
*/
	...
	{
		q->f_index = nextIndex(q->f_index);		//move front index to the next element
	}
}
...
*/
	...
	{
		queueFront(q, c);											//obtain the value of element in front of the queue
		q->f_index = nextIndex(q->f_index);		//move front index to the next element
	}
}
...
*/
	...
	if (queueFull(q) != 0)								//execute error function if queue is full
		queueError(QERR_UP);
	else
	{
		q->arr[q->b_index] = c;							//write data
		q->b_index = nextIndex(q->b_index);	//move free position indicator to next element
	}
}
