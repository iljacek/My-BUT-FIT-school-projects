...
**/
  ...
  if((item) == NULL)                                            //strom je prázdny, môžeme rovno vložiť
  {
  ...
  else
  {
    while (item)
    {
      if ((item->Cont < Content) && (item->RPtr != NULL))       //ak je hodnota vacsia a ma praveho potomka, skontroluj praveho potomka
        item = item->RPtr;
      else if ((item->Cont > Content) && (item->LPtr != NULL))  //ak je hodnota mensia a ma laveho potomka, skontroluj laveho potomka
        item = item->LPtr;
      else if(item->Cont == Content)                            //ak sa prvok nasiel, ukonci funkciu
        return;
      else
        {
          tBTNodePtr newItem = malloc(sizeof(struct tBTNode));  //ak sme dosli na koniec cesty v strome, vieme ze mozeme vlozit novy prvok
          newItem->RPtr = NULL;
          newItem->LPtr = NULL;
          newItem->Cont = Content;
          if(item->Cont < Content)                              //rozhodneme ci prvok bude zdedeny v lavej alebo pravej vetve
            item->RPtr = newItem;
          else
            item->LPtr = newItem;
        }
...
**/
  ...
  if(RootPtr == NULL)                       //ak je strom prázdny, ukonči vykonávanie funkcie
    return;

  tBTNodePtr item = RootPtr;
  tStackP stack;                            //vytvorenie a inicializacia stacku pointerov
  SInitP(&stack);
  Leftmost_Preorder(item, &stack);
  while (!SEmptyP(&stack))                  //kým nie je zasobník praźdny popuje hodnoty z vrcholu a hlada najlavsi uzol
  {
    item = STopPopP(&stack);
...
**/
  ...
  while (!SEmptyP(&stack))                  //kým nie je zasobník prázdny popuje hodnoty z vrcholu a hlada najlavsi uzol
  {
    item = STopPopP(&stack);
    BTWorkOut(item);                        //spracovanie uzla
...
**/
  ...
  tBTNodePtr item = RootPtr;
  bool left;
  tStackP stackP;                              //vytvorenie a inicializacia stacku pointerov
  tStackB stackB;                              //a BOOL stacku,ktory znaci, ci sa uzlom už prešlo
  SInitP(&stackP);
  SInitB(&stackB);
  Leftmost_Postorder(item, &stackP, &stackB);
  while (!SEmptyP(&stackP))                    //kým nie je zasobník prázdny popuje hodnoty z vrcholu a hlada najlavsi
  {
    item = STopPopP(&stackP);
    SPushP(&stackP, item);
    left = STopPopB(&stackB);
    if (left)                                  //prichadza z lava, pojde doprava
    {
      SPushB(&stackB,FALSE);
      Leftmost_Postorder(item->RPtr, &stackP, &stackB);
    }
    else                                       //prichadza z prava, odstráni a zpracuje uzol
    {
      STopPopP(&stackP);
      BTWorkOut(item);
    }
...
if(*RootPtr == NULL)                                 //ak je strom prázdny, ukonči vykonávanie funkcie
  ...
  do {
    if (*RootPtr == NULL)                           // ak koreň neexistuje
    {
      if (!SEmptyP(&stack))                         //ak je stack prazdny uložím hodnotu a popnem
        *RootPtr = STopPopP(&stack);
    }
    else
    {
      if ((*RootPtr)->RPtr != NULL)                 // pravého dám do zásobníku
        SPushP(&stack,(*RootPtr)->RPtr);
      tBTNodePtr item = *RootPtr;
      *RootPtr = (*RootPtr)->LPtr;                  // posúvam sa vľavo
      free(item);                                   // zruším aktuálny uzol
    }
